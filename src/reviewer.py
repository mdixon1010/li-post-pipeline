...
def review_drafts_openai(posts: List[str]) -> str:
    """Selects the best post from three provided drafts using OpenAI's GPT model.

    This function reads a system prompt configuration from a YAML file, constructs
    a user message containing the drafts, and sends the input to OpenAI's chat 
    completions API to determine the best draft.
    ...
    Raises:
        ValueError: If `posts` does not contain exactly three non-empty strings.
        FileNotFoundError: If the YAML configuration file is not found.
        KeyError: If the `reviewer_system_message` key is missing in the YAML file.
    ...
    if not openai_api_key:
        ...
        raise EnvironmentError("Missing API key for OpenAI. Set the OPENAI_KEY environment variable.")

    # Validate input
    if not isinstance(posts, list) or len(posts) != 3 or not all(isinstance(post, str) and post.strip() for post in posts):
        logging.error("Invalid input: `posts` must be a list of exactly three non-empty strings.")
        raise ValueError("`posts` must be a list of exactly three non-empty strings.")
    ...
    try:
        ...
        raise FileNotFoundError(f"Configuration file not found at: {YML_CONFIG}")
    ...
    user_message = (
        f"""Given the three posts below (each between the <post> tags) output the best post word for word.
